...
export default function LegalProfilePage() {
  ...
  const fetchProfile = async () => {
    try {
      const response = await fetch("/api/profile/legal");
      if (!response.ok) {
      ...
      if (data.hotels && data.hotels.length > 0) {
        ...
        setEditData({
          ...
          responsiblePersonName: hotel.responsiblePersonName || "",
          password: "",
        });
      }
    } catch (err) {
  ...
  // Sections for legal person
  const sections = [
    { 
      id: "dailySheets",
      label: "დღის ფურცლები",
      description: "ნახეთ და დაადასტურეთ დღის ფურცლები",
      path: "/legal/daily-sheets"
    },
    { 
      id: "invoices",
      label: "ინვოისები",
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setEditData({ ...editData, identificationCode: e.target.value })}
                  className="w-full border rounded px-3 py-2"
                />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">პასუხისმგებელი პირი</label>
                <input
                  type="text"
                  value={editData.responsiblePersonName || ""}
                  onChange={(e) => setEditData({ ...editData, responsiblePersonName: e.target.value })}
                  className="w-full border rounded px-3 py-2"
                />
              </div>
              <div>
