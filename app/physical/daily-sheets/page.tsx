...
export default function PhysicalDailySheetsPage() {
  ...
  const loadAvailableMonths = async () => {
    try {
      const response = await fetch("/api/physical/daily-sheets");
  ...
  const fetchDailySheets = async () => {
    ...
    try {
      let url = "/api/physical/daily-sheets";
      if (selectedDay) {
        url += `?day=${selectedDay}`;
  ...
  const confirmDailySheet = async (sheetId: string) => {
    try {
      const response = await fetch(`/api/physical/daily-sheets?id=${sheetId}`, {
        method: "PUT",
      });
  ...
  const handleDelete = async (sheetId: string) => {
    ...
    try {
      const response = await fetch(`/api/physical/daily-sheets/${sheetId}`, {
        method: "DELETE",
      });
      ...
      await Promise.all([fetchDailySheets(), loadAvailableMonths()]);
      alert("დღის ფურცელი წარმატებით წაიშალა");
    } catch (err) {
