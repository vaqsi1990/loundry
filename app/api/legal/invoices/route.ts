...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get legal invoice payments for this user
    const legalPayments = await (prisma as any).legalInvoicePayment.findMany({
      where: {
        userId: session.user.id,
      },
    });

    // Update paid amounts from legal payments
    const paymentMap = new Map<string, { paidAmount: number; isPaid: boolean }>();
    legalPayments.forEach((payment: any) => {
