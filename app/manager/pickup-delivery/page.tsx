...
export default function ManagerPickupDeliveryPage() {
  ...
  const fetchRequests = async () => {
    try {
      const response = await fetch("/api/manager/pickup-delivery");
  ...
  const handleConfirm = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`/api/manager/pickup-delivery/${requestId}/confirm`, {
        method: "PUT",
      });
      if (!response.ok) {
  ...
  const handleDelete = async (requestId: string) => {
    ...
    try {
      const response = await fetch(`/api/manager/pickup-delivery/${requestId}`, {
        method: "DELETE",
      });
      if (!response.ok) {
        const errorData = await response.json();
